...
import com.kams.service.ComlistService;
import com.kams.service.UserService;
...
public class UserController {
	...
	public String deleteUserById(@RequestBody String param) { // axios传递json串
		// {"users":["songz","caoche"]}
		System.out.println(param);
		JsonReqObject jsonReqObject = JSONArray.parseObject(param, JsonReqObject.class);
		String userId = jsonReqObject.getMsg();
	...
	public String updateUser(@RequestBody String param) { // axios传递json串
		// {"msg":["songz","caoche"],key:普通用户}
		JsonReqObject jsonReqObject = JSONArray.parseObject(param, JsonReqObject.class);
		String[] msg = jsonReqObject.getMsg().replace("\\", "").replace("}\",", "}\";").replace("[\"", "")
				.replace("\"]", "").split("\";\"");
		List<SysUser> userlist = new ArrayList<SysUser>();
		for (String string : msg) {
